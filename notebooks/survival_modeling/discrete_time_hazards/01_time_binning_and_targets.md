...
For a patient with observed time
```math
X_i
```
, define:
...
\delta_i=1
```
, the event happened in interval
```math
\kappa_i
```
. Then:
...
\delta_i=0
```
, the patient is censored in interval
```math
\kappa_i
```
. We know:
...
The discrete survival probability through the end of interval
```math
k
```
is:
...
Each time bin has its own readout vector
```math
w_k
```
. Therefore:
...
is the slide evidence for risk in interval
```math
k
```
.
